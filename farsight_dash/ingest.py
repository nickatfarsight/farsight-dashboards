...
def read_annotations(config, shared_dirs):
    """Read the Weekly Chart Annotations CSV (launches / promos per fiscal week).

    Optional. Columns: year, week, type, channel, label, date.
    Returns a list of {yr, wk, type, channel, label, date} dicts (possibly empty).
    """
    ann_cfg = config.get('sources', {}).get('annotations', {})
    pattern = (ann_cfg or {}).get('file_pattern', 'Weekly Chart Annotations')
    fp = find_file(pattern, shared_dirs)
    if not fp:
        return []

    print(f"  Reading annotations: {os.path.basename(fp)}")
    out = []
    try:
        df = pd.read_csv(fp)
        for _, row in df.iterrows():
            try:
                yr = int(row['year']); wk = int(row['week'])
            ...
            out.append({
                'yr': yr,
                'wk': wk,
                'type': safe_str(row.get('type')) or 'note',
                'channel': safe_str(row.get('channel')) or 'All',
                'label': label,
                'date': safe_str(row.get('date')),
            })
        print(f"  Loaded {len(out)} annotations")
    except Exception as e:
        print(f"  Error reading annotations: {e}")
    return out


def read_daash(config, shared_dirs):
    """Read competitive market-intel CSV (brand × week B&M sales, TY + LY).

    Optional. Columns: brand, index, week, ty_sales, ly_sales.
    Returns a list of row dicts (possibly empty).
    """
    daash_cfg = config.get('sources', {}).get('daash', {})
    pattern = (daash_cfg or {}).get('file_pattern', 'BM Sales by Week')
    fp = find_file(pattern, shared_dirs)
    if not fp:
        return []
    print(f"  Reading DAASH competitive data: {os.path.basename(fp)}")
    out = []
    try:
        df = pd.read_csv(fp)
        for _, row in df.iterrows():
            ...
            out.append({
                'brand': brand,
                'index': safe_str(row.get('index')) or '',
                'week': wk,
                'ty': num(row.get('ty_sales', 0)),
                'ly': num(row.get('ly_sales', 0)),
            })
        print(f"  Loaded {len(out)} DAASH rows")
    except Exception as e:
